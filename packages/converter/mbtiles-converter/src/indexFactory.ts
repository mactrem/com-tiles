...
export async function createIndexInRowMajorOrder(
  ...
  for (let zoom = minZoom; zoom <= maxZoom; zoom++) {
    ...
    const numTiles =
      (limits.maxTileRow - limits.minTileRow + 1) *
      (limits.maxTileCol - limits.minTileCol + 1);
      ...
      for (let row = 0; row < numFragmentsRow; row++) {
        for (let col = 0; col < numFragmentsCol; col++) {
          ...
          Object.assign(fragmentBounds, {
            ...
            maxTileCol:
              fragmentBounds.maxTileCol + numIndexEntriesPerFragmentSide,
          });
        }
        ...
        fragmentBounds = {
          ...
          maxTileCol:
            (fragmentMinColIndex + 1) * numIndexEntriesPerFragmentSide - 1,
          maxTileRow:
            fragmentBounds.maxTileRow + numIndexEntriesPerFragmentSide,
        };
      }
    }
  }
...
function isDense(
  ...
  return (
    fragmentBounds.minTileCol >= limits.minTileCol &&
    fragmentBounds.minTileRow >= limits.minTileRow &&
    fragmentBounds.maxTileCol <= limits.maxTileCol &&
    fragmentBounds.maxTileRow <= limits.maxTileRow
  );
}
