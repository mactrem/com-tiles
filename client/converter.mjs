...
export function calculateIndexOffsetForTile(metadata, zoom, x, y) {
    var _a;
    const tileMatrixSet = metadata.tileMatrixSet;
    const supportedOrdering = [undefined, SUPPORTED_ORDERING];
    if (![tileMatrixSet.fragmentOrdering, tileMatrixSet.tileOrdering].
        ...
        if (ts.zoom < zoom) {
            ...
            * */
            ...
            if (ts.aggregationCoefficient === -1) {
                ...
                const fragmentLimitsInTiles = calculateFragmentBounds(ts.tileMatrixLimits, { minTileCol, minTileRow, maxTileCol, maxTileRow });
...
function calculateFragmentBounds(tileSetLimits, denseFragmentLimits) {
    const intersectedLimits = Object.assign({}, denseFragmentLimits);
...
function calculateDenseTileSetFragmentBounds(numTilePerFramgentSide, tileSetLimits) {
    const denseFragmentTileSetLimits = Object.assign({}, tileSetLimits);
